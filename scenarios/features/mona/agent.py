...
class Agent(BaseAgent):
        ...
        for i, task in enumerate(self.tasks_info):
            if not task.completed:
                ...
                if val_y is not None and float(val_y) > 0:
                    compressed_y[i] = int(round(float(val_y) * BID_SCALE_FACTOR))
    ...
    def local_message_receive(self):
        """
        Restore compressed messages and pass to CBBA.
        (Original restore logic maintained + type safety enhanced)
        """
        now = time.time()
        peer_ids = set()
        ...
        if isinstance(monitor, dict):
            ...
            for k, v in recv.items():
                if isinstance(v, dict) and "y" in v:
                    ...
                    # 1. Restore Winning Bids - type check enhanced
                    y_raw = v["y"]
                    ...
                    if "s" in v and v["s"]:
                        ...
                        if isinstance(compressed_s, dict):
                            restored_msg["message_received_time_stamp"] = {
                                str(ag_id): (int(ts) + TIMESTAMP_OFFSET)  # <--- str로 변경 (ESP-NOW 버전과 동일하게)
                    ...
                    # Process message only if agent_id is valid
                    if restored_msg["agent_id"] is not None:
                        self.receive_message(restored_msg)
                        try:
                            peer_ids.add(int(restored_msg["agent_id"]))
                        except (ValueError, TypeError):
                            pass  # Ignore if agent_id cannot be converted to integer

        # Update cache if new messages received
        if peer_ids:
